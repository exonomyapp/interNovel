...
#### 4.2. The "Character Suitcase": A Novel Collaborative Paradigm
...
* **Suitcase Contents (Prepared by Character's Original Author):**
...
* **Simplified Real-Time Collaboration & Awareness (MVP1):**
    ...
    * **Near Real-Time Updates:** Changes made by one author to shared data (e.g., Spine content, Suitcase status, permissions) will be reflected for other collaborators with minimal delay. This will be facilitated by Supabase's real-time database capabilities (e.g., subscribing to database changes via WebSockets).
...
* **Character Showcase:** A dedicated area where authors can publicly or semi-publicly list their original characters, complete with their "Character Suitcases" (bio, thematic preferences, writing samples from the character's original author).
...
* **User-Choice for Document Storage:** While InterNovel manages metadata (including Spine data, Character bios, Suitcase info, Casting Studio/Novel Café listings, **theme/topic/section** definitions) and collaborative states, users will have the option to connect their accounts from third-party services (e.g., GitHub, GitLab, Google Docs) to store the actual text files of their novels.
...
* User adoption and active engagement (e.g., number of registered users, stories created, "Character Suitcases" exchanged, characters listed in Casting Studio, novels showcased in Novel Café, follow/subscriptions initiated, Novel Spines utilized, themes/topics defined).
